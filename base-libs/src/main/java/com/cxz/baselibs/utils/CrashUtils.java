...
public class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = BaseApp.getInstance()
                    .getPackageManager()
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && BaseApp.getInstance().getExternalCacheDir() != null)
            defaultDir = BaseApp.getInstance().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = BaseApp.getInstance().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
